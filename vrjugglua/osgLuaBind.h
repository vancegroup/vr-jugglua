/**	@file
        @brief	header with template specializations providing
   Luabind-compatible
...
#define OSGLUABIND_VERBOSE(X)                                                  \
    std::cout << "[osgLuaBind.h:" << __LINE__ << "] " << X << std::endl
#else
#define OSGLUABIND_VERBOSE(X)
#endif
...
namespace luabind {
    namespace detail {
        ...
        /// reference) -
        /// uses a type-dispatched partial specialized function for the "inner
        /// section"
        ...
        static int compute_osglua_score_for_type(lua_State *L, int index) {
            ...
            static const osgLua::introspection::Type &destType =
                osgLua::introspection::Reflection::getType(
                    extended_typeid<T>());
    ...
    struct osglua_ref_converter_base
        ...
        /// luabind because that
        /// library assumes that you won't make a converter for pointer types.
        /// These are minimal
...
namespace luabind {
    ...
    struct default_converter<
        T *, typename boost::enable_if<typename boost::is_base_and_derived<
                 detail::osg_ref_base_t,
                 T>::type>::type> : osglua_ref_converter_base<T> {};

    /// Anything held in an osg::ref_ptr should be converted as an osgLua
    /// reference object
    template <typename T>
    struct default_converter< ::osg::ref_ptr<T> > : osglua_ref_converter_base<
        T, ::osg::ref_ptr<T> > {};
    ...
    namespace detail {
        ...
        /// luabind::detail::osg_ref_base_t
        /// includes annotation mentioning osgLuaBind and type name from
        /// osgIntrospection.
        ///
        ...
        struct type_to_string<
            T, typename boost::enable_if<typename boost::is_base_of<
                   detail::osg_ref_base_t, T>::type>::type> {
            static void get(lua_State *L) {
        ...
        /// modifiers like *, &,
        /// and const, so no need to define those here.
        template <typename T> struct type_to_string<osg::ref_ptr<T> > {
            static void get(lua_State *L) {
...
namespace luabind {
    ...
    struct default_converter<
        T,
        typename boost::enable_if<IsOSGValue<typename boost::remove_const<
            typename boost::remove_reference<T>::type>::type> >::
            type> : osglua_val_converter_base<typename boost::
                                                  remove_const<
                                                      typename boost::
                                                          remove_reference<T>::
                                                              type>::type> {};
    namespace detail {
        ...
        template <typename T>
        struct type_to_string<
            T, typename boost::enable_if<IsOSGValue<T> >::type
            /// typename boost::enable_if< boost::remove_const< typename
            /// boost::remove_reference<IsOSGValue<T> >::type >::type > >::type>
            > {
            static void get(lua_State *L) {
...
namespace boost {
    /// Anything defined as an OSG Value does not inherit from the osg base
    /// referenced object type
    // BOOST_TT_AUX_BOOL_TRAIT_PARTIAL_SPEC2_1(typename T, is_base_and_derived,
    // ::luabind::detail::osg_ref_base_t, T, luabind::IsOSGValue<T>::value)
}
...
/// that
/// bound item that reads something like "No matching overload found,
/// candidates:
...
/// that type's
/// header, but forward declaration here keeps this header from forcing every
/// file
/// using it to include lots of extra headers.
///
/// Any value types may be added here to make their converter available
/// automatically when
/// including this file and the corresponding osg header. To make a converter
/// available
/// without modifying this main header, use the CREATE_OSGLUA_VALUE_CONVERTER
/// macro.
///
/// @{
