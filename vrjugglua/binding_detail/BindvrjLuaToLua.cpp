...
namespace vrjLua {
    ...
    static void appendToModelSearchPath(std::string const &path) {
        ...
        if (p.empty()) {
            throw std::runtime_error(
                "Can't append an empty string to the model search path!");
        }
    ...
    static void appendToLuaRequirePath(lua_State *L, std::string const &path) {
        ...
        if (p.empty()) {
            throw std::runtime_error(
                "Can't append an empty string to the require path!");
        }
    ...
    void BindvrjLuaToLua(lua_State *L) {
#ifdef VERBOSE
        std::cerr << "Registering vrjLua module functions with Lua..."
                  << std::flush << std::endl;
#endif
        module(
            L,
            "vrjLua")[def("appendToModelSearchPath", &appendToModelSearchPath),
        ...
        if (!shareDir.empty()) {
            std::cout << "Found VR JuggLua share directory (" << shareDir
                      << "), adding to model search path..." << std::endl;
            appendToModelSearchPath(shareDir);
        }

        std::string const &rootDir = LuaPath::instance().getRootDir();
        if (!rootDir.empty()) {
            std::cout << "Found root directory (" << rootDir
                      << "), adding to model search path..." << std::endl;
            appendToModelSearchPath(rootDir);

            std::cout << "Adding config directory (" << rootDir
                      << "/etc/vrjugglua/) to Lua package path..." << std::endl;
