...
namespace vrjLua {
    ...
    bool KernelState::hasInitialized() { return instance()._init; }

#if __VJ_version >= 2003000
    // Juggler 3.x - initialize the kernel
    inline boost::program_options::options_description
    getVrjOptionsDescriptions() {
        boost::program_options::options_description desc(
            "VR Juggler 3.x Options");
        desc.add(vrj::Kernel::instance()->getClusterOptions())
            .add(vrj::Kernel::instance()->getConfigOptions());
        return desc;
    }
    ...
    void KernelState::init(int argc, char *argv[]) {
        boost::program_options::options_description &cluster_desc =
            vrj::Kernel::instance()->getClusterOptions();
    ...
    void KernelState::init() {
        VRJLUA_MSG_START(dbgVRJLUA, MSG_WARNING)
            << "Warning: your application is not cluster-capable as compiled "
               "(VR Juggler 3.x) because you did not initialize the kernel "
               "with command line arguments!"
            << VRJLUA_MSG_END(dbgVRJLUA, MSG_WARNING);
        boost::program_options::variables_map vm;
        // Juggler assumes these will be in a provided variable map, and gives
        // an unhelpful boost::any_cast exception
        // if not set.
        vm.insert(std::make_pair(
            std::string("vrjmaster"),
            boost::program_options::variable_value(false, true)));
        vm.insert(std::make_pair(
            std::string("vrjslave"),
            boost::program_options::variable_value(false, true)));
        init(vm);
    ...
    void KernelState::init() {
        // no-op
        VRJLUA_MSG_START(dbgVRJLUA, MSG_WARNING)
            << "Warning: your application will not cluster-capable with VR "
               "Juggler 3.x because you did not initialize the kernel with "
    ...
    namespace {
        ...
        void doStart(lua_State *L, bool waiting = false) {
            if (!KernelState::hasInitialized()) {
                VRJLUA_MSG_START(dbgVRJLUA, MSG_WARNING)
                    << "Warning: vrjKernel.start called before vrjKernel.init "
                       "- your app will not support clustering in VR Juggler "
                       "3.x!" << VRJLUA_MSG_END(dbgVRJLUA, MSG_WARNING);
                KernelState::init();
            }

            if (!waiting) {
                VRJLUA_MSG_START(dbgVRJLUA, MSG_WARNING)
                    << "Warning: vrjKernel.start called: possible race "
                       "condition if you run Lua code in the frame loop! (Use "
                       "vrjKernel.enter to avoid this)"
                    << VRJLUA_MSG_END(dbgVRJLUA, MSG_WARNING);
            }

            // Override os.exit to actually just call kernel stop.
            luabind::globals(L)["os"]["sysexit"] =
                luabind::globals(L)["os"]["exit"];
            luabind::globals(L)["os"]["exit"] =
                luabind::globals(L)["vrjKernel"]["stop"];

            vrj::Kernel::instance()->start();
        }

        void start(lua_State *L) { doStart(L); }

        void stop() {
            if (vpr::Thread::self() != vrj::Kernel::instance()->getThread()) {
                /// We are not the kernel control thread - do a sleep here
                /// to reduce race condition issues with quick startup/shutdown
                /// (like in tests)

                VRJLUA_MSG_START(dbgVRJLUA, MSG_STATUS)
                    << "vrjKernel.stop() called from outside the control "
                       "thread - sleeping for 2 sec for thread safety"
        ...
        void setApplication(luabind::object app) {
            vrj::Kernel::instance()->setApplication(
                luabind::object_cast<KernelOsgApp *>(app));
        }
        ...
        bool isRunning() { return vrj::Kernel::instance()->isRunning(); }

    } // end of namespace

    void bindKernelToLua(lua_State *L) {
#ifdef VERBOSE
        std::cerr << "Registering vrjKernel module functions with Lua..."
                  << std::flush << std::endl;
#endif
        module(L, "vrjKernel")
            [def("enter", &enter), def("start", &start), def("stop", &stop),
             ...
             def("initAsClusterPrimaryNode",
                 &KernelState::initAsClusterPrimaryNode),
             def("initAsClusterSecondaryNode",
                 &KernelState::initAsClusterSecondaryNode),
             def("initAsClusterSecondaryNode",
                 &initAsClusterSecondaryNodeDefaultPort),
