...
namespace vrjLua {
        ...
        if (L) {
            ...
            module(L, "vrjApp")
                [class_<OsgAppProxy>("OsgAppProxy")
                     ...
                     .def("setActiveApplication",
                          &OsgAppProxy::setActiveApplication)
    ...
    luabind::object const &OsgAppProxy::getAppDelegate() { return _delegate; }
    ...
    void OsgAppProxy::initScene() {
        ...
        if (!_delegate) {
            VRJLUA_MSG_START(dbgVRJLUA_PROXY, MSG_WARNING)
                << "No delegate has been set yet - exiting to avoid "
        ...
        xform->setScale(
            osg::Vec3d(FEET_PER_METER, FEET_PER_METER, FEET_PER_METER));
        _rootNode = xform.get();

        VRJLUA_MSG_START(dbgVRJLUA_PROXY, MSG_STATUS)
            << "Number of children before forwarding call to delegate: "
            << _rootNode->getNumChildren()
            << VRJLUA_MSG_END(dbgVRJLUA_PROXY, MSG_STATUS);

        _forwardCallToDelegate("initScene",
                               true); // required call - exceptions propagate

        VRJLUA_MSG_START(dbgVRJLUA_PROXY, MSG_STATUS)
            << "Number of children after forwarding call: "
            << _rootNode->getNumChildren()
            << VRJLUA_MSG_END(dbgVRJLUA_PROXY, MSG_STATUS);
    }
    ...
    void OsgAppProxy::configSceneView(osgUtil::SceneView *newSceneViewer) {
        ...
        */
        ...
        newSceneViewer->getGlobalStateSet()->setAttributeAndModes(lightmodel,
    ...
    void OsgAppProxy::preFrame() {
        if (!_delegationSuccessFlag) {
            ...
            VRJLUA_MSG_START(dbgVRJLUA_PROXY, MSG_ERROR)
                << "A full frame loop completed without any successful calls "
                   "to the delegate."
                << VRJLUA_MSG_END(dbgVRJLUA_PROXY, MSG_ERROR);
            VRJLUA_MSG_START(dbgVRJLUA_PROXY, MSG_ERROR)
        ...
        if (vrj::Kernel::instance()->getUsers().size() > 0) {
            cur_time =
                vrj::Kernel::instance()->getUsers()[0]->getHeadUpdateTime();
        }
    ...
    void OsgAppProxy::intraFrame() { _forwardCallToDelegate("intraFrame"); }

    void OsgAppProxy::postFrame() { _forwardCallToDelegate("postFrame"); }
    ...
    osg::Group *OsgAppProxy::getScene() { return _rootNode.get(); }

    double OsgAppProxy::getTimeDelta() { return _timeDelta; }
    ...
    bool OsgAppProxy::_forwardCallToDelegate(const char *call, bool required) {
        if (_delegate && luabind::type(_delegate[call]) == LUA_TFUNCTION) {
            try {
                ...
                std::stringstream s;
                s << "Calling '" << call
                  << "' in the app delegate failed - check your lua code!"
                  << std::endl;
        ...
        VRJLUA_MSG_START(dbgVRJLUA_PROXY, MSG_DRIVEL)
            << "Delegate not valid or no '" << call
            << "' element defined: " << _delegate
