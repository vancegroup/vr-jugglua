...
namespace vrjLua {
    ...
    inline bool LuaRunBuffer::_full() const { return _buf.full(); }

    inline bool LuaRunBuffer::_empty() const { return _buf.empty(); }

    inline bool LuaRunBuffer::ready() const { return _script.isValid(); }
...
#ifdef RUNBUFFER_VERBOSE
        VRJLUA_MSG_START(dbgVRJLUA_BUFFER, MSG_STATUS)
            << "Done writing." << VRJLUA_MSG_END(dbgVRJLUA_BUFFER, MSG_STATUS);
#endif
    }
    ...
    void LuaRunBuffer::readObject(vpr::ObjectReader *reader) {
        ...
        if (_buf.size() != _maxRun) {
            VRJLUA_MSG_START(dbgVRJLUA_BUFFER, MSG_WARNING)
                << "Upon reading serialized data, buffer sizes don't match! "
                   "Remote has " << _maxRun << " but we have " << _buf.size()
