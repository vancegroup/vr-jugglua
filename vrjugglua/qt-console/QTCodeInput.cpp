...
namespace vrjLua {
    ...
    void QTCodeInput::dragEnterEvent(QDragEnterEvent *e) {
        if (e->mimeData()->hasUrls()) {
            ...
            for (int i = 0; i < urlList.size(); ++i) {
                /// @TODO handle files that aren't jconfs or lua files by just
                /// putting their name.
    ...
    bool QTCodeInput::isJconf(QUrl const &url) {
        return /*url.isLocalFile() url.scheme() == "file" && */ url
            .toLocalFile()
            .endsWith(QString(".jconf"), Qt::CaseInsensitive);
    }
    bool QTCodeInput::isLocalLuaFile(QUrl const &url) {
        return /*url.isLocalFile() url.scheme() == "file" && */ url
            .toLocalFile()
            .endsWith(QString(".lua"), Qt::CaseInsensitive);
    }
