...
namespace osgLua {
    namespace detail {
        ...
        template <typename T> struct LuaEquivalent;
        ...
        template <> struct LuaEquivalent<double> : is_lua_Number {};

        template <> struct LuaEquivalent<float> : is_lua_Number {};

        template <> struct LuaEquivalent<int> : is_lua_Integer {};

        template <> struct LuaEquivalent<unsigned int> : is_lua_Integer {};

        /// Forward declaration of templated way to push values into Lua
        template <typename T> struct ToLua;

        /// We know how to push lua_Integer
        template <> struct ToLua<lua_Integer> {
        ...
        template <> struct ToLua<lua_Number> {
        ...
        template <> struct ToLua<const char *> {
        ...
        template <> struct ToLua<const std::string> {
    ...
    Value::Value(const introspection::Value &v) : _value(v) {
        ...
        static const introspection::Type &t_referenced =
            introspection::Reflection::getType(
                extended_typeid<osg::Referenced>());
            ...
            if (type.isNonConstPointer()) {
                ...
                if (p_type.isDefined())
                    if (p_type.isSubclassOf(t_referenced)) {
                        _referenced =
                            introspection::variant_cast<osg::Referenced *>(
                                _value);
                    }
            }
        }
    ...
    introspection::Value getValue(lua_State *L, int index,
        ...
        if (lua_isstring(L, index)) {
            return introspection::Value(std::string(lua_tostring(L, index)));
        }

        if (lua_isboolean(L, index)) {
            return introspection::Value((lua_toboolean(L, index) == 0) ? false
                                                                       : true);
        }
    ...
    void Value::_pushWrappedValue(lua_State *L,
                                  const introspection::Value &original) {
        // create a userdata
