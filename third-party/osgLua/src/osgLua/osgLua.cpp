...
namespace osgLua {
    namespace detail {
        ...
        inline int explicitNumericType(lua_State *L) {
            ...
            if (!lua_isnumber(L, 1)) {
                return luaL_error(
                    L, "Expected a number as the first and only parameter!");
            }
            lua_Number x = lua_tonumber(L, 1);
            Value::_pushWrappedValue(
                L, introspection::Value(static_cast<ValueType>(x)));
            return 1;
        }
...
bool osgLua::loadWrapper(lua_State *L, const char *name) {
    ...
    if (lua_isnil(L, -1)) {
        ...
        try {
            ...
            std::cout << "Failed to load wrapper for " << name << " : "
                      << e.what() << std::endl;
            return false;
        }
            ...
            try {
                ...
                if (tcheck != typeid(osg::Vec3d)) {
                    std::cerr << "\n "
                                 ...
                                 "*********************************************"
                                 "*************"
                                 "\n * osgWrapper was not correctly  loaded. "
                                 "osgIntrospection *"
                                 "\n * requires in unix systems to be loaded "
                                 "with RTLD_GLOBAL *"
                                 "\n * flag enabled, but  currently lua "
                                 "standard distribution *"
                                 "\n * doesn't use it.                         "
                                 "               *"
                                 "\n *                                         "
                                 "               *"
                                 "\n * You can use osgLua interpreter (in "
                                 "osgLua/examples) it *"
                                 "\n * is  the standard  lua interpreter  "
                                 "linked with osgLua, *"
                                 "\n * that sovles the problem.                "
                                 "               *"
                                 "\n *                                         "
                                 "               *"
                                 "\n * If you have lua embedded  in your "
                                 ...
                                 "\n *                                         "
                                 "               *"
                                 "\n * We expect lua developers solve this "
                                 "issue some time in *"
                                 "\n * the future.                             "
                                 "               *"
                                 "\n "
                ...
                std::cerr << "Could not get type info for osg::Vec3d in "
                          << __FILE__ << ":" << __LINE__
        ...
        osglib **ptr =
            reinterpret_cast<osglib **>(lua_newuserdata(L, sizeof(osglib *)));
...
int luaopen_libosgLua(lua_State *L) { return luaopen_osgLua(L); }
