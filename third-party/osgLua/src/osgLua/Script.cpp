...
namespace osgLua {
    ...
    void Script::execute(const std::string &code, introspection::ValueList *vl,
        ...
        execute(luaL_loadbuffer(L, code.c_str(), code.length(), aux.c_str()),
                vl);
    }

    bool Script::call(const std::string &funcName,
                      const introspection::ValueList *params,
    ...
    void Script::set(const introspection::Value &val, const std::string &name) {
        Value::push(L, val);
