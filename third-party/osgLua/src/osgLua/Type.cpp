...
namespace osgLua {
        ...
        try {
            ...
            /** @todo remove this old enum code?
                                    ...
                                    if (type.isEnum())
                                    {
                                            ...
                                            const introspection::EnumLabelMap
               &map =
                                                    type.getEnumLabels();
                                            for(
               introspection::EnumLabelMap::const_iterator
                                                    i = map.begin(); i !=
               map.end(); ++i)
                                            {
                                                    if (i->second == method) {
               ...
               os;
                                                            ...
                                                            type.getReaderWriter()->readTextValue(s,
               v);
            ...
            */
            ...
            luaL_error(L, "Class %s do not have static method %s", tname,
                       method);
        }
    ...
    int Type::index(lua_State *L) {
        ...
        try {
            ...
            if (type.isEnum()) {
                // create a table with the values
                const introspection::EnumLabelMap &map = type.getEnumLabels();
                // Type::push(L, base.c_str());

                lua_newtable(L);
                for (introspection::EnumLabelMap::const_iterator i =
                         map.begin();
                     i != map.end(); ++i) {
            ...
            return luaL_error(L, "Type '%s' does not have a wrapper defined, "
                                 "so it is not accessible through osgLua.",
                              base.c_str());
        }
        catch (introspection::Exception &e) {
            return luaL_error(L, "osgLua introspection error:  %s",
                              e.what().c_str());
        }
    }
