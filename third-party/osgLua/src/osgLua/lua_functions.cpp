...
namespace osgLua {
    ...
    void pushParameter(lua_State *L, const introspection::ParameterInfo *p) {
        ...
        lua_pushfstring(L, "[%s] %s %s", type, getName(p->getParameterType()),
                        p->getName().c_str());
    }

    void pushMethodInfo(lua_State *L, const introspection::MethodInfo *mi) {
        int top = lua_gettop(L);
        lua_pushfstring(L, "%s\t%s(", getName(mi->getReturnType()),
                        mi->getName().c_str());
        const introspection::ParameterInfoList &param = mi->getParameters();
        for (introspection::ParameterInfoList::const_iterator i = param.begin();
             i != param.end(); ++i) {
            pushParameter(L, *i);
            lua_pushstring(L, ", ");
        }
    ...
    void pushArrayPropertyInfo(lua_State *L,
                               const introspection::PropertyInfo *pi) {
        lua_pushstring(L, getName(pi->getPropertyType()));
        lua_pushstring(L, "\t");

        lua_pushstring(L, pi->getName().c_str());

        lua_pushstring(L, "[i]\t(array - access through 1-based index, the # "
                          "operator for size, or method calls :insert)");
    }

    void pushIndexedPropertyInfo(lua_State *L,
                                 const introspection::PropertyInfo *pi) {
        ...
        lua_pushstring(L, "(");
        const introspection::ParameterInfoList &param =
    ...
    void pushSimplePropertyInfo(lua_State *L,
                                const introspection::PropertyInfo *pi) {
    ...
    void pushPropertyInfo(lua_State *L, const introspection::PropertyInfo *pi) {
        ...
        if (pi->isArray()) {
            ...
            /// @todo indexed properties not yet supported - no sense in showing
            /// them
            lua_pushstring(L, " ");
            ...
            // should not happen
            luaL_error(L,
                       "%s:%d Property is neither array, indexed, nor simple!",
                       __FILE__, __LINE__);
        }
    ...
    void pushConstructorInfo(lua_State *L,
        ...
        int top = lua_gettop(L);
        lua_pushfstring(L, "%s (", getName(ci->getDeclaringType()));
        ...
        if (type) {
            ...
            }
            {
                ...
                try {
                    ...
                    luaL_error(L, "Caught exception %s in type named %s",
                               e.what().c_str(),
