...
namespace osgLua {
    ...
    ArrayPropertyProxy::ArrayPropertyProxy(
        introspection::Value const &instance,
    ...
    const char *ArrayPropertyProxy::_getMethodRegistryString() {
        static std::string regString =
            std::string(typeid(ArrayPropertyProxy).name()) + ":MethodTable";
    ...
    int ArrayPropertyProxy::index(lua_State *L) {
        ...
        if (argType == LUA_TSTRING) {
            ...
            try {
                ...
                return luaL_error(
                    L, "[%s:%d] Indexed property '%s' caught exception: %s",
        ...
        // lua_pushnil(L);
        // return 1;
        return luaL_error(L, "[%s:%d] Indexed property '%s' expected access of "
                             "element by index, in 1 through %d, or a method "
                             "call to 'insert'",
                          __FILE__, __LINE__, _propName.c_str(),
                          _propInfo->getNumArrayItems(_instance));
    }
    ...
    int ArrayPropertyProxy::newindex(lua_State *L) {
        ...
        if (eltIndex < n && eltIndex >= 0) {
            try {
                ...
                return luaL_error(L, "[%s:%d] Can't assign given value to "
                                     "index %d: got OSG exception '%s'",
        ...
        if (eltIndex == n) {
            try {
                ...
                return luaL_error(
                    L, "[%s:%d] Can't append given value at the end of array "
                       "(index %d): got OSG exception '%s'",
        ...
        if (n == 0) {
            return luaL_error(L, "[%s:%d] Can't assign to index %d: array has "
                                 "0 elements, and not assigning to index 1",
                              __FILE__, __LINE__, eltIndex + 1);
        }
        return luaL_error(L, "[%s:%d] Can't assign to index %d: out of "
                             "existing range 1-%d, and not contiguous at end "
                             "location %d",
                          __FILE__, __LINE__, eltIndex + 1, n, n + 1);
    }
    ...
    int ArrayPropertyProxy::len(lua_State *L) {
        ...
        try {
            ...
            return luaL_error(L, "[%s:%d] Caught exception trying to get "
                                 "length of property '%s': %s",
                              __FILE__, __LINE__, _propName.c_str(),
                              e.what().c_str());
        }
    }
    ...
    int ArrayPropertyProxy::insert(lua_State *L) {
        ...
        switch (lua_gettop(L)) {
            ...
            case 3:
                ...
                break;
            default:
                return luaL_error(L, "[%s:%d] Indexed property '%s' call to "
                                     "'insert' expected at least a value, and "
                                     "potentially a location and a value",
                                  __FILE__, __LINE__,
                                  _propInfo->getName().c_str());
        }
        ...
        try {
            ...
            return luaL_error(L, "[%s:%d] Caught exception trying to insert "
                                 "item into property '%s': %s",
                              __FILE__, __LINE__, _propName.c_str(),
                              e.what().c_str());
        }
    }
    ...
    int ArrayPropertyProxy::remove(lua_State *L) {
        ...
        int i =
            luaL_optinteger(L, 2, _propInfo->getNumArrayItems(_instance)) - 1;
        if (_pushItemAtArrayIndex(L, i)) {
        ...
        return luaL_error(
            L, "[%s:%d] Can't remove item %d from array property %s: out of "
               "range. (Contains %d elements, starting at 1)",
            __FILE__, __LINE__, i + 1, _propInfo->getName().c_str(),
            _propInfo->getNumArrayItems(_instance));
    }

    void ArrayPropertyProxy::registerAdditionalMetamethods(lua_State *L) {
        Base::NonConstInstanceMethod::registerMetamethod<
            &ArrayPropertyProxy::index>(L, "__index");
        Base::NonConstInstanceMethod::registerMetamethod<
            &ArrayPropertyProxy::newindex>(L, "__newindex");
        Base::NonConstInstanceMethod::registerMetamethod<
            &ArrayPropertyProxy::len>(L, "__len");
        luaL_newmetatable(L, _getMethodRegistryString());
        Base::NonConstInstanceMethod::pushInstanceMethod<
            &ArrayPropertyProxy::insert>(L);
        lua_setfield(L, -2, "insert");
        Base::NonConstInstanceMethod::pushInstanceMethod<
            &ArrayPropertyProxy::remove>(L);
