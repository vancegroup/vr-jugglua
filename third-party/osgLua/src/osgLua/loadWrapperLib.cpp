...
#define OSGLUA_NOTIFY(level)                                                   \
    if (osg::isNotifyEnabled(level))                                           \
    osg::notify(level)
#endif
...
namespace {
    ...
    class LibNamePossibilityList {
      public:
        ...
        const_iterator begin() const { return _names.begin(); }

        iterator begin() { return _names.begin(); }

        const_iterator end() const { return _names.end(); }
        ...
        void pushNameVariants(std::string const &basename) {
            ...
            if (basename.find(LIB) == 0) {
                // Starts with lib already - try stripping it. Starting at
                // sizeof(LIB)
                // is one too far, due to null terminator.
                const std::string noLib =
                    basename.substr(sizeof(LIB) - 1, std::string::npos);
                OSG_INFO << "Varying basename by stripping " << LIB << ": "
                         << noLib << std::endl;
                pushDirectoryVariants(noLib);
            } else {
                OSG_INFO << "Varying basename by adding a prefix: " << LIB
                         << basename << std::endl;
      ...
      private:
        static std::string getPluginDirectoryPrepend() {
            static const std::string prepend =
                "osgPlugins-" + std::string(osgGetVersion()) + "/";
            return prepend;
        }
...
osgDB::DynamicLibrary *loadWrapperLib(std::string const &libname) {
    ...
    OSG_INFO << std::endl
             << "Using our own code to compute a (possibly redundant) guess."
             << std::endl;
    names.pushNameVariants(createLibraryNameForWrapper(libname));

    for (LibNamePossibilityList::const_iterator it = names.begin(),
                                                e = names.end();
         it != e; ++it) {
        /// @todo replace the inside of this loop with
        /// osgDB::Registry::instance()->loadLibrary()
        /// This would let OSG manage library lifetimes for us - a mixed
        /// blessing, since it does it
        /// globally, but probably better than right now, where we'll joyfully
        /// crash if you
