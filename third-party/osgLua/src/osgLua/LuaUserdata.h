...
#define LUA_USERDATA_VERBOSE(X)                                                \
    { std::cerr << "LuaUserdata.h:" << __LINE__ << ":\t" << X << std::endl; }
#else
#define LUA_USERDATA_VERBOSE(X)                                                \
    {}
#endif
...
#define LUA_USERDATA_STACKCHECKER(NAME, STATE, EXPECTEDDIFF)                   \
    StackChecker<> NAME(STATE, __FILE__, __LINE__, EXPECTEDDIFF)
#else
#define LUA_USERDATA_STACKCHECKER(NAME, STATE, EXPECTEDDIFF)
#endif
...
namespace luacpputils {
    ...
    /// other C++ classes.
    template <typename Derived> class LuaUserdata : private LuaUserdataBase {
      public:
      ...
      private:
        ...
        static void _pushMetatable(lua_State *L) {
            ...
            if (luaL_newmetatable(L, _getRegistryString())) {
                LUA_USERDATA_VERBOSE(
                    "Userdata type "
                    << _getRegistryString << ":\t"
                    << "Registering garbage collection metamethod");
                lua_pushvalue(L, -1);
                lua_pushcfunction(L, &_gc);
                lua_setfield(L, -2, gcMetamethodName()); /// table is one below
                                                         /// the top of the
                                                         /// stack
        ...
        static void _pushIndexTable(lua_State *L) {
            ...
            if (lua_isnil(L, -1)) {
                ...
                lua_newtable(L);
                lua_pushvalue(
                    L, -1); /// now the top two values are the __index table
                lua_setfield(L, metatableIndex, indexMetamethodName());
            } else if (!lua_istable(L, -1)) {
                luaL_error(L, "While registering an instance method of %s: "
                              "__index metamethod already set to value of type "
                              "%s!",
                           _getRegistryString(), lua_typename(L, -1));
                return;
            }
            /// OK, past this point we have the __index table at -1.

            /// Remove the metatable from the stack so that all we have pushed
            /// is the __index table
            lua_remove(L, -2);
        }
        ...
        static int _gc(lua_State *L) {
            ...
            LUA_USERDATA_VERBOSE("Userdata " << instance << ":\t"
        ...
        static PointerToDerivedType
        _allocateInLua(lua_State *L, PointerToDerivedType instance) {
            LUA_USERDATA_STACKCHECKER(checker, L, 1);
            LUA_USERDATA_VERBOSE("Userdata " << instance << ":\t"
                                             << "Created instance of type "
                                             << _getRegistryString());
      ...
      protected:
        template <typename PtrToMemberFuncType> class InstanceMethodHandler {
          private:
            ...
            static int _callInstanceMethod(lua_State *L) {
                void *instancePtr = luaL_checkudata(L, 1, _getRegistryString());
                if (!instancePtr) {
                    return luaL_error(L, "Trying to call an instance method of "
                                         "%s, but first argument is not an "
                                         "instance!",
                                      _getRegistryString());
                }
                PointerToDerivedType instance;
                std::memcpy(&instance, instancePtr,
                            sizeof(PointerToDerivedType));

                LUA_USERDATA_VERBOSE(
                    "Userdata "
                    << instance << ":\tMethod " << _getMethodDescription<M>()
                    << "\t"
                    << "Before instance method call with lua_gettop(L)=="
                    << lua_gettop(L));
                /// Leaving the instance on the stack in case the method wants
                /// access to it.
                int ret = ((*instance).*(M))(L);
                LUA_USERDATA_VERBOSE("Userdata "
                                     << instance << ":\tMethod "
                                     << _getMethodDescription<M>() << "\t"
                                     << "After instance method call returning "
                                     << ret);
                return ret;
            }
          ...
          public:
            ...
            static void pushInstanceMethod(lua_State *L, int upvalues = 0) {
                LUA_USERDATA_STACKCHECKER(checker, L, 1);
                LUA_USERDATA_VERBOSE("Userdata type "
                                     << _getRegistryString() << ":\tMethod "
                                     << _getMethodDescription<M>() << "\t"
                                     << "Pushing an instance method");
                lua_pushcclosure(L, &_callInstanceMethod<M>, upvalues);
            }

            template <PtrToMemberFuncType M>
            static void registerMetamethod(lua_State *L,
                                           const char *metamethodName) {
                LUA_USERDATA_STACKCHECKER(checker, L, 0);
                LUA_USERDATA_VERBOSE("Userdata type "
                                     << _getRegistryString << ":\tMethod "
                                     << _getMethodDescription<M>() << "\t"
                                     << "Registering metamethod "
                                     << metamethodName);
                _pushMetatable(L);
                pushInstanceMethod<M>(L);
                lua_setfield(L, -2, metamethodName); /// table is one below the
                                                     /// top of the stack
                lua_pop(L, 1); /// pop the metatable off the stack.
            }

            template <PtrToMemberFuncType M>
            static void registerObjectMethod(lua_State *L,
                ...
                _pushIndexTable(L);
                pushInstanceMethod<M>(L);
                lua_setfield(L, -2, methodName); /// table is one below the top
                                                 /// of the stack
                lua_pop(L, 1); /// pop the __index table off the stack.
            }
        };

        typedef InstanceMethodHandler<NonConstInstanceMethodPtrType>
        NonConstInstanceMethod;
        typedef InstanceMethodHandler<ConstInstanceMethodPtrType>
        ConstInstanceMethod;
