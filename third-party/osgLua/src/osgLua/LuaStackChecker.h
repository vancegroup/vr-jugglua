...
namespace luacpputils {
    namespace StackCheckerErrorPolicies {
        /// @brief Error policy for StackChecker that handles errors by raising
        /// a Lua error.
        struct LuaError {
            static void reportError(lua_State *L, const char *file, int line,
                                    int top, int diff, int currentTop) {
                if (file) {
                    luaL_error(L, "StackChecker error for scope opened at "
                                  "%s:%d: Expected top to be %d + %d, got %d",
                               file, line, top, diff, currentTop);
                } else {
                    luaL_error(L, "StackChecker error: Expected top to be %d + "
                                  "%d, got %d",
                               top, diff, currentTop);
                }
            }
        ...
        /// @brief Error policy for StackChecker that handles errors by using a
        /// C assert.
        struct Assert {
            static void reportError(lua_State *, const char *, int, int top,
                                    int diff, int currentTop) {
    ...
    /** @brief RAII-style verification that you're leaving the Lua stack
            ...
            @tparam ErrorPolicy determines the behavior when a mis-match is
       found.
    */
    ...
    class StackChecker {
      public:
        ...
        */
        StackChecker(lua_State *L, const char *file, int line,
                     int difference = 0)
            : _L(L)
            , _diff(difference)
            , _file(file)
        ...
        ~StackChecker() {
            ...
            if (currentTop != _top + _diff) {
                ErrorPolicy::reportError(_L, _file, _line, _top, _diff,
                                         currentTop);
            }
        }
