...
namespace osgLua {
    ...
    int console_print(lua_State *L) {
        Console *console =
            reinterpret_cast<Console *>(lua_touserdata(L, lua_upvalueindex(1)));
    ...
    Console::Console(unsigned int nl)
        : _numLines(nl)
        , _current(nl - 1) {
        ...
        for (unsigned int i = 0; i < _numLines; ++i) {
            ...
            _text[i]->setPosition(
                osg::Vec3(0, (i + 1) * 1.0f / (_numLines + 1), 0));
        }
