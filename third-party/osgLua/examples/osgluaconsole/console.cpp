	...
	virtual bool handle(const osgGA::GUIEventAdapter& ea,osgGA::GUIActionAdapter&)
	{
		switch(ea.getEventType())
		{
			case(osgGA::GUIEventAdapter::KEYDOWN):
				{
					int key = ea.getKey();
					if (key >0 && key < 256) _console->addInputChar(key);
...
void Console::_addText(const std::string &s, Console::TextType tt)
{
	...
	{
		case CONSOLE_NORMAL:
			_text[_current]->setColor(osg::Vec4(1,1,0,1)); break;
		case CONSOLE_ERROR:
			_text[_current]->setColor(osg::Vec4(1,0,0,1)); break;
		case CONSOLE_COMMAND:
			_text[_current]->setColor(osg::Vec4(1,1,1,1)); break;
	}
